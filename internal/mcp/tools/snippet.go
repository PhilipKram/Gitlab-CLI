...
func registerSnippetView(server *mcp.Server, f *cmdutil.Factory) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		if err := requireID(in.Snippet, "snippet"); err != nil {
			return nil, nil, err
		}
		client, err := f.Client()
		if err != nil {
...
func registerSnippetCreate(server *mcp.Server, f *cmdutil.Factory) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		if err := requireString(in.Title, "title"); err != nil {
			return nil, nil, err
		}
		if err := requireString(in.Filename, "filename"); err != nil {
			return nil, nil, err
		}
		if err := requireString(in.Content, "content"); err != nil {
			return nil, nil, err
		}
		client, err := f.Client()
		if err != nil {
