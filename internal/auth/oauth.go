...
func OAuthFlow(host, clientID, redirectURI, scopes string, out io.Writer, openBrowser func(string) error) (*Status, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("starting local server on %s: %w\nPort may be in use. Close the process using it or set a different redirect URI with --redirect-uri", listenAddr, err)
	}
	...
	if browserOpened {
		...
		_, _ = fmt.Fprintf(out, "! Open this URL in your browser to authenticate:\n  %s\n", authURL)
	}
	_, _ = fmt.Fprintf(out, "- Waiting for authentication...\n")
