...
func PrintUpdateNotice(out io.Writer, currentVersion string) bool {
	...
	_, _ = fmt.Fprintf(out, "Run `glab upgrade` to update, or download from:\n")
...
// ValidateAssetURL validates that a URL points to an expected GitHub domain.
// This prevents downloading from untrusted sources during self-update.
func ValidateAssetURL(rawURL string) error {
	...
	if parsed.Scheme != "https" {
		return fmt.Errorf("asset URL must use HTTPS, got %s", parsed.Scheme)
	}

	// Validate hostname is github.com or api.github.com
	hostname := strings.ToLower(parsed.Hostname())
	validDomains := []string{"github.com", "api.github.com"}
	...
	return fmt.Errorf("asset URL must point to github.com or api.github.com, got %s", hostname)
}
...
func DownloadAsset(url, destDir string) (string, error) {
	// Validate URL before downloading to prevent supply chain attacks
...
func VerifyChecksum(archivePath, checksumURL string) error {
	if checksumURL == "" {
		return fmt.Errorf("checksum verification is mandatory but no checksums.txt asset was found in the release")
	}
