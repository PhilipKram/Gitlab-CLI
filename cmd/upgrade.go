  ...
  $ glab upgrade --yes`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case update.InstallBrew:
				_, _ = fmt.Fprintln(out, "glab was installed via Homebrew. To upgrade, run:")
				_, _ = fmt.Fprintln(out, "  brew upgrade glab")
				return nil
			case update.InstallDeb:
				_, _ = fmt.Fprintln(out, "glab was installed via a Debian package. To upgrade, download the latest .deb from:")
				_, _ = fmt.Fprintln(out, "  https://github.com/PhilipKram/Gitlab-CLI/releases")
				return nil
			case update.InstallRPM:
				_, _ = fmt.Fprintln(out, "glab was installed via an RPM package. To upgrade, download the latest .rpm from:")
				_, _ = fmt.Fprintln(out, "  https://github.com/PhilipKram/Gitlab-CLI/releases")
				return nil
			}
			...
			if !result.IsNewer && !force {
				_, _ = fmt.Fprintf(out, "glab is already up to date (v%s)\n", version)
				return nil
			}

			_, _ = fmt.Fprintf(out, "Current version: v%s\n", version)
			_, _ = fmt.Fprintf(out, "Latest version:  v%s\n", result.LatestVersion)
			...
			// Extract binary
			_, _ = fmt.Fprintln(out, "Extracting binary...")
			...
			_, _ = fmt.Fprintf(out, "Successfully upgraded glab to v%s\n", result.LatestVersion)
			return nil
		},
	}
